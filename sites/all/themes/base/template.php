...
function base_preprocess_node(&$vars){
	...
	if($vars['type'] == 'post'){
		...
		if($vars['view_mode'] == 'teaser'){
			...
			//$image = $vars['field_cover_image'];
			//$vars['cover_image'] = l(base_render_image($image, 'post_full', $url, ''), $url, array('html' => TRUE));
			//$vars['cover_image'] = base_render_image($image, 'post_full', $url, '');
		}
